...
function ResetPassword() {
  ...
  const [success, setSuccess] = useState(false) // Estado para a mensagem profissional
  ...
  useEffect(() => {
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event) => {
      if (event === "PASSWORD_RECOVERY") {
        ...
        // Se o usuário não estiver em fluxo de recuperação, redireciona
        if (!success) {
  ...
  async function handleReset(e) {
    ...
    if (error) {
      ...
      // Em vez de apenas o alert, ativamos a tela de sucesso
      setSuccess(true)
      // Opcional: manter o alert para navegadores antigos ou apenas o redirecionamento
      setTimeout(() => {
        navigate('/login')
      }, 3500) // 3.5 segundos para o usuário ler a mensagem
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-voel-beige px-4">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-sm border border-gray-100 text-center">
        ...
        {success ? (
          // --- INTERFACE DE SUCESSO PROFISSIONAL ---
          <div className="animate-in fade-in zoom-in duration-500 py-6">
            ...
            <h2 className="font-serif text-xl tracking-[0.2em] text-voel-charcoal uppercase">Senha Atualizada</h2>
