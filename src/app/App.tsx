...
// --- DADOS DOS PRODUTOS ---
// Adicionada a propriedade isFeatured para controlar o que aparece em Destaques e ganha o selo "Exclusivo"
...
function Header({ onSearchChange, favoritesCount, onShowFavorites, onPageChange, activePage, activeCategory }) {
      ...
      >
        <div className="container mx-auto px-4 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className={`flex items-center bg-[#F5F1EC]/80 rounded-full px-3 py-1.5 transition-all duration-500 ease-in-out ${isSearchOpen ? 'w-32 sm:w-64 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
            ...
            <nav className="flex-1 overflow-y-auto p-10 space-y-10">
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-6">
                  {categories.map(cat => {
                    const isItemSelected = (cat === activeCategory);
                    return (
                      <button 
                        key={cat}
              ...
              <div className="pt-10 border-t border-[#E5E0D8]/30 space-y-6">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <button onClick={onShowFavorites} className="block text-sm uppercase tracking-[0.3em] w-full text-left text-[#3D3835] hover:text-[#8B7355] transition-all hover:translate-x-2">
                    Favoritos
                  </button>
                  <button 
...
export default function App() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (showFavorites) items = items.filter(p => favorites.includes(p.id));
    // Se a categoria for Destaques, filtra apenas produtos isFeatured
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF7] text-[#3D3835] selection:bg-[#8B7355]/20 font-sans">
      <Header
        ...
        onShowFavorites={() => { setShowFavorites(!showFavorites); setActivePage('loja'); }}
              ...
              <section id="vitrine" className="container mx-auto px-4 py-20">
                <div className="text-center mb-16">
                  <h2 className="text-3xl font-serif uppercase tracking-[0.3em] mb-3">
                    {showFavorites ? 'Meus Favoritos' : (activeCategory === 'Todos' ? 'Coleção Completa' : activeCategory)}
                  </h2>
                  <p className="text-[#9B8F7E] text-[10px] uppercase tracking-[0.5em] font-light">Peças exclusivas selecionadas para você</p>
                ...
                {filteredProducts.length > 0 ? (
                    ...
                    {filteredProducts.map(p => (
                      <div key={p.id} className="group cursor-pointer">
                        <div className="relative aspect-[3/4] overflow-hidden bg-[#F5F1EC] mb-6 rounded-2xl shadow-sm border border-[#E5E0D8]/30">
                          ...
                          {/* O selo Exclusivo agora só aparece se o produto for Destaque (isFeatured) */}
                          {p.isFeatured && (
                  ...
                  <div className="py-20 text-center space-y-4">
                    ...
                    <button onClick={() => {setShowFavorites(false); setActiveCategory('Todos');}} className="text-[#8B7355] text-xs uppercase tracking-widest underline underline-offset-4">Ver toda a coleção</button>
                  </div>
                )}
